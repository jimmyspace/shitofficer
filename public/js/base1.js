 ...
 */
var isIos = false;
var base_url = "http://yuxiaobao.1an.com";
var app_id = "";
var key = "";
var pagesize = 10;
...
function get_ShowYuxiaobao() {
    ...
    return (date == null ? new Date('2001/01/01').format("yyyy/MM/dd") : date)
}

function oauth() {
    window.localStorage.setItem("redirect_url", window.location.href);
    var url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc9cd44f46e2351a5&redirect_uri=" + base_url + "/app_wx/_oauto.html&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect";
...
//去掉支付时候的支付中的层 6-22
function showLoaderEmpty() {
    $("div.pupInfo").remove(); $("div.pupBg").remove();
    $("body").append("<div class='pupBg' style='display:block'></div>");
}
...
function loadData(objList) {
    ...
    $(".btnUp a").click(function () { if ($(".btnUp a").html() != "全部加载完成！") GetData(false); })
...
//ajax调用数据 返回json
function ajaxPost_Json_Sync(url, data, callback) {
    $.ajax({
...
function wx_config(callback) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var d = data.split('|');
            var jsapi_appId = d[1];
            var jsapi_timestamp = d[2];
            var jsapi_nonceStr = d[3];
            var jsapi_signature = d[4];
